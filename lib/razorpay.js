    ...
    try {
      ...
      const options = {
        ...
        modal: {
          ondismiss: async () => {
            try {
              await releasePaymentHold(order.bookingId, order.orderId);
            } catch (releaseError) {
              console.error("Unable to release Razorpay hold", releaseError);
            }
            onStatus?.("");
      ...
      razorpay.on("payment.failed", async (response) => {
        try {
          ...
          console.error("Unable to release failed-payment hold", releaseError);
        }
...
async function releasePaymentHold(bookingId, orderId) {
  ...
  if (!response.ok) throw new Error("Unable to release payment hold.");
