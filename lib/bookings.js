...
export async function confirmPaidBooking(id, paymentId) {
  ...
  if (booking.status === "confirmed" && booking.razorpay_payment_id === paymentId) return booking;
  if (["cancelled", "expired"].includes(booking.status)) {
    throw new Error("Cannot confirm a cancelled or expired booking.");
  }
...
export async function releasePendingBooking(id, reason = "") {
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase
    .from("bookings")
    .update({ status: "cancelled", error_message: reason || null, updated_at: new Date().toISOString() })
    .eq("id", id)
    .eq("status", "payment_pending")
    .select("id, status")
    .maybeSingle();
