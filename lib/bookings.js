  ...
  if (error) {
    if (error.code === "23505") {
      const conflict = new Error(
        "That appointment slot is currently being booked by another customer. Please choose another time."
      );
...
/**
 * Confirm a successfully paid booking.
 *
 * Idempotent:
 * Razorpay can deliver the same payment/webhook more than once.
 */
export async function confirmPaidBooking(id, paymentId) {
  ...
  if (["cancelled", "expired"].includes(booking.status)) {
    throw new Error(
      "Cannot confirm a cancelled or expired booking."
    );
  }
...
/**
 * Release a temporary payment hold.
 *
 * Only payment_pending bookings can be released.
 * Confirmed/paid bookings are not touched.
 */
...
export async function getHeldSlots(date) {
  ...
  const { data, error } = await supabase
    .from("bookings")
    .select(
      "appointment_time, status, hold_expires_at"
    )
  ...
  return (data || [])
    .filter(
      (row) =>
        ...
        (
          row.hold_expires_at &&
          DateTime.fromISO(row.hold_expires_at) > now
        )
    )
    .map((row) =>
...
export async function getAvailableSlots(
  ...
  return generateSlots(date).filter(
    (slot) => !unavailable.has(slot.start)
  );
}